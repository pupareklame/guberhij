...
/**
 * [INTEGRITY-CHECK]: 0x7065726c756173
 * STATUS: PROTECTED-V1
 */

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Maximize, Download, RefreshCw, Sparkles, Image as ImageIcon, Zap, X, Check, Scissors, Send } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState } from '../types';
import { expandImage, upscaleImage } from '../services/perluas';
import ImageUploader from '../components/ImageUploader';
import { useTheme } from '../src/contexts/ThemeContext';

const GuberPerluas: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setProcessing({ isProcessing: true, error: null, progress: 'Expanding Image...' });
    setResultImage(null);

    try {
      const result = await expandImage(sourceImage, { expansionType: 'AUTO', additionalInstruction: prompt });
      setResultImage(result);
      setProcessing({ isProcessing: false, error: null, progress: '' });
    } catch (err: any) {
      setProcessing({ isProcessing: false, error: err.message || "Gagal meluaskan foto.", progress: '' });
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `perluas-foto-${Date.now()}.png`;
    link.click();
  };
    ...
    <div className="h-full bg-slate-50/50 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto min-h-full bg-white flex flex-col border-x border-slate-100">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Perluas Foto AI</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Neural Outpainting Engine</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Send size={14} className="text-slate-300" /> 2. Instruksi Perluas
            </label>
            <textarea 
              value={prompt} 
              onChange={(e) => setPrompt(e.target.value)} 
              placeholder="Misal: Tambahkan pemandangan pegunungan di sisi kiri dan kanan..." 
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 text-[11px] font-bold text-slate-700 focus:border-teal-500 outline-none transition-all h-[120px] resize-none shadow-inner placeholder:text-slate-300" 
            />
          </div>
          ...
          <div className="">
            ...
            >
              ...
              {processing.isProcessing ? (
                <span className="relative z-10">EXPANDING...</span>
              ) : (
                <span className="text-lg relative z-10">LUASKAN FOTO</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Perluas
              </label>
            </div>
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Hasil Perluas</h2>
