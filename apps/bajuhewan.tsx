...
/**
 * [INTEGRITY-CHECK]: 0x62616a75686577616e
 * STATUS: PROTECTED-V1
 */

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Dog, Download, RefreshCw, Sparkles, Image as ImageIcon, Zap, X, Check, Scissors, Palette, Settings2 } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState } from '../types';
import { generateAnimalKid, upscaleImage } from '../services/bajuhewan';
...
const ANIMAL_PRESETS = [
  ...
  { id: 'DINOSAURUS', name: 'Dino', icon: '🦖' },
  { id: 'GAJAH', name: 'Gajah', icon: '🐘' },
  { id: 'KUCING', name: 'Kucing', icon: '🐱' },
];

const GuberAnimal: React.FC = () => {
  const { primaryColor } = useTheme();
  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [selectedAnimalId, setSelectedAnimalId] = useState<string | null>(ANIMAL_PRESETS[0].id);
  const [customAnimalName, setCustomAnimalName] = useState<string>('');
  const [isChangeBgEnabled, setIsChangeBgEnabled] = useState<boolean>(false);
  const [selectedEnv, setSelectedEnv] = useState<string>('Studio Foto Estetik');
  ...
  const handleGenerate = async () => {
    if (!sourceImage) {
      setProcessing(prev => ({ ...prev, error: "Harap unggah foto anak terlebih dahulu." }));
      return;
    }

    const animalName = customAnimalName.trim() || 
      (ANIMAL_PRESETS.find(a => a.id === selectedAnimalId)?.name) || 'Singa';

    const envToUse = isChangeBgEnabled 
      ? (customBgPrompt.trim() || selectedEnv) 
      : '';

    setProcessing({ isProcessing: true, error: null, progress: 'AI sedang menjahit kostum...' });
    setResultImage(null);

    try {
      const result = await generateAnimalKid(sourceImage, animalName, envToUse, '');
      setResultImage(result);
      setProcessing({ isProcessing: false, error: null, progress: '' });
    } catch (err: any) {
      setProcessing({ isProcessing: false, error: err.message || "Gagal membuat kostum.", progress: '' });
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `kostum-hewan-${Date.now()}.png`;
    link.click();
  };
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Kostum Hewan AI</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Neural Costume Engine</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <ImageIcon size={14} className="text-slate-300" /> 1. Unggah Foto Anak
            </label>
            <ImageUploader 
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Palette size={14} className="text-slate-300" /> 2. Pilih Kostum Hewan
            </label>
            <div className="grid grid-cols-4 gap-2">
              {ANIMAL_PRESETS.map((animal) => (
                <button
                  key={animal.id}
                  onClick={() => { setSelectedAnimalId(animal.id); setCustomAnimalName(''); }}
                  className={`flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all duration-300 aspect-square ${
                    selectedAnimalId === animal.id && !customAnimalName
                      ? 'scale-105 shadow-sm' 
                      : 'border-slate-100 bg-slate-50/50 text-slate-400 hover:border-slate-200 hover:bg-white'
                  }`}
                  style={selectedAnimalId === animal.id && !customAnimalName ? {
                    backgroundColor: primaryColor,
                    color: 'white',
                    borderColor: primaryColor,
                  } : {}}
                >
                  <span className="text-xl mb-1">{animal.icon}</span>
                  <span className="text-[8px] font-black uppercase truncate w-full text-center leading-tight">{animal.name}</span>
                </button>
              ))}
            </div>
            <input 
              ...
              onChange={(e) => { setCustomAnimalName(e.target.value); setSelectedAnimalId(null); }}
              placeholder="Atau ketik hewan kustom..."
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <Settings2 size={14} className="text-slate-300" /> 3. Latar Belakang
              </label>
              <label className="relative inline-flex items-center cursor-pointer scale-75">
                <input type="checkbox" className="sr-only peer" checked={isChangeBgEnabled} onChange={() => setIsChangeBgEnabled(!isChangeBgEnabled)} />
            ...
            <AnimatePresence>
              {isChangeBgEnabled && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4 overflow-hidden">
                  <select value={selectedEnv} onChange={(e) => setSelectedEnv(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-3 text-[11px] font-bold text-slate-700 focus:border-teal-500 outline-none appearance-none">
                    <option value="Studio Foto Estetik">Studio Foto Estetik</option>
                    <option value="Hutan Fantasi Berwarna">Hutan Fantasi</option>
                    <option value="Kamar Tidur Anak Lucu">Kamar Tidur Anak</option>
                    <option value="Taman Bunga Matahari">Taman Bunga</option>
                  </select>
                  <textarea value={customBgPrompt} onChange={(e) => setCustomBgPrompt(e.target.value)} placeholder="Atau ketik latar kustom..." className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-3 text-[11px] font-bold text-slate-700 focus:border-teal-500 outline-none min-h-[80px] resize-none placeholder:text-slate-300 shadow-inner" />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="">
            ...
            >
              ...
              {processing.isProcessing ? (
                <span className="relative z-10">TAILORING...</span>
              ) : (
                <span className="text-lg relative z-10">GENERATE KOSTUM</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Kostum
              </label>
            </div>
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Kostum</h2>
