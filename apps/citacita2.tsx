...
import { generateCitaCita2 } from '../services/citacita2';
...
const DEFAULT_POSE = 'half-body shot, arms crossed over chest or upper stomach, body slightly tilted, face facing camera, cool expression with a slight smile';

const GuberCitaCita2: React.FC = () => {
  ...
  const [config, setConfig] = useState<CitaCitaConfig>({
    dreamJob: 'DOKTER',
    pose: DEFAULT_POSE,
    environment: 'ENVIRONMENT_FOLLOW_JOB',
    gender: 'PRIA',
    style: 'FOTOREALISTIK',
    aspectRatio: '9:16',
    additionalPrompt: '',
  ...
  const handleProcessCitaCita = async () => {
    ...
    setProcessing({ isProcessing: true, error: null, progress: 'Mewujudkan Impian...' });

    try {
      const result = await generateCitaCita2(image, config, selectedEngine);
      setResultImage(result);
      setInitialResultImage(result);
      setProcessing({ isProcessing: false, error: null, progress: '' });
    } catch (err: any) {
      setProcessing({ isProcessing: false, error: err.message || "Gagal mewujudkan cita-cita.", progress: '' });
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `citacita-${Date.now()}.png`;
    link.click();
  };
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">CITA-CITA V2</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60 text-center">Outfit & Career Swap</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex-1 flex flex-col min-h-0">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar lg:pr-1 space-y-6">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Gaya Visual</label>
                    <div className="grid grid-cols-2 gap-2">
                       {STYLES.map(s => (
                         <button 
                          key={s.id}
                          onClick={() => setConfig({...config, style: s.id as any})}
                          className={`py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${config.style === s.id ? 'bg-slate-900 text-white border-transparent' : 'bg-white border-slate-200 text-slate-500'}`}
                         >
                           {s.name}
                         </button>
                       ))}
                  ...
                  <div className="space-y-1.5 pt-4 border-t border-slate-100">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                      <Zap size={10} className="text-amber-500" /> Enjin AI (Model)
                    </label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { id: 'gemini-2.5-flash-image', name: 'Lite', desc: 'Cepat' },
                        { id: 'gemini-3-flash-image-preview', name: '3 Flash', desc: 'Sangat Detail' }
                      ].map((engine) => (
                        <button
                          key={engine.id}
                          onClick={() => setSelectedEngine(engine.id)}
                          className={`flex flex-col items-center justify-center py-2 rounded-xl border-2 transition-all duration-300 gap-0.5 ${
                        ...
                        >
                          <span className="text-[8px] font-black uppercase">{engine.name}</span>
                    ...
                    {processing.isProcessing ? (
                      ...
                      >
                        ...
                        <p className="mt-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] animate-pulse">
                          {processing.progress || 'Menciptakan Masa Depan...'}
                        </p>
                      </motion.div>
                    ) : resultImage ? (
                      <motion.div
        ...
        {isCropping && resultImage && (
          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm text-center">Crop Visualisasi</h2>
