...
/**
 * [INTEGRITY-CHECK]: 0x6f6c6168666f746f
 * STATUS: PROTECTED-V1
 */

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Sparkles, Download, RefreshCw, Image as ImageIcon, Zap, X, Check, Scissors, Palette, Film } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState } from '../types';
import { restorePhoto, upscaleImage } from '../services/olahfoto';
import ImageUploader from '../components/ImageUploader';
import { useTheme } from '../src/contexts/ThemeContext';

const GuberRestore: React.FC = () => {
  const { primaryColor } = useTheme();
  const [sourceImage, setSourceImage] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [mode, setMode] = useState<'FULL' | 'BW_ONLY'>('FULL');
  ...
  const handleRestore = async () => {
    if (!sourceImage) {
      setProcessing(prev => ({ ...prev, error: "Unggah foto lama terlebih dahulu." }));
      return;
    }

    setProcessing({ isProcessing: true, error: null, progress: 'Neural Photo Recovery...' });
    setResultImage(null);

    try {
      const result = await restorePhoto(sourceImage, mode, customInstruction, aspectRatio);
      setResultImage(result);
      setProcessing({ isProcessing: false, error: null, progress: '' });
    } catch (err: any) {
      setProcessing({ isProcessing: false, error: err.message || "Gagal merestorasi foto.", progress: '' });
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `restore-foto-${Date.now()}.png`;
    link.click();
  };
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Restorasi AI</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Neural Photo Recovery</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <ImageIcon size={14} className="text-slate-300" /> 1. Unggah Foto Lama
            </label>
            <ImageUploader 
              label="Pilih Foto Sumber" 
              image={sourceImage} 
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Palette size={14} className="text-slate-300" /> 2. Pilih Mode Restorasi
            </label>
            <div className="grid grid-cols-1 gap-2">
              <button 
                onClick={() => setMode('FULL')} 
                className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${mode === 'FULL' ? 'scale-[1.02] shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-slate-200'}`}
                style={mode === 'FULL' ? { 
              ...
              >
                <div className={`p-2 rounded-lg ${mode === 'FULL' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-400'}`}>
                  <Palette size={18} />
                </div>
                <div className="text-left">
                  <p className="text-[10px] font-black uppercase tracking-widest">Ultimate Color Restore</p>
                  <p className={`text-[8px] font-bold ${mode === 'FULL' ? 'text-white/70' : 'opacity-60'}`}>Restorasi penuh warna & detail HD</p>
                </div>
              </button>
              <button 
                onClick={() => setMode('BW_ONLY')} 
                className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all ${mode === 'BW_ONLY' ? 'scale-[1.02] shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-slate-200'}`}
                style={mode === 'BW_ONLY' ? { 
                  backgroundColor: primaryColor,
                  borderColor: primaryColor,
                  color: 'white'
              ...
              >
                <div className={`p-2 rounded-lg ${mode === 'BW_ONLY' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-400'}`}>
                  <Film size={18} />
                </div>
                <div className="text-left">
                  <p className="text-[10px] font-black uppercase tracking-widest">Classic HD B&W</p>
                  <p className={`text-[8px] font-bold ${mode === 'BW_ONLY' ? 'text-white/70' : 'opacity-60'}`}>Restorasi tajam hitam putih</p>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              placeholder="Contoh: Jadikan gaun warna biru, tambahkan background bintang, pertajam mata..."
              className="w-full px-4 py-3 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:border-teal-500 outline-none transition-all text-[11px] font-bold text-slate-700 placeholder:text-slate-300 resize-none"
              rows={3}
            />
          </div>
          ...
          <div className="">
            ...
            >
              ...
              {processing.isProcessing ? (
                <span className="relative z-10">RECONSTRUCTING...</span>
              ) : (
                <span className="text-lg relative z-10">PROSES RESTORE</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Restorasi
              </label>
            </div>
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Hasil Restorasi</h2>
