...
/**
 * [INTEGRITY-CHECK]: 0x62657273616d61
 * STATUS: PROTECTED-V1
 */

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Users, Download, RefreshCw, Sparkles, Image as ImageIcon, Zap, X, Check, Scissors, MapPin } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState, GabungConfig } from '../types';
import { generateGabungScene, upscaleImage } from '../services/bersama';
import ImageUploader from '../components/ImageUploader';
import { useTheme } from '../src/contexts/ThemeContext';

const GuberGabung: React.FC = () => {
  const { primaryColor } = useTheme();
  const [subject1Image, setSubject1Image] = useState<string | null>(null);
  const [subject2Image, setSubject2Image] = useState<string | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [sliderPos, setSliderPos] = useState(50);
  const [aspectRatio, setAspectRatio] = useState('9:16');
  
  const [config, setConfig] = useState<GabungConfig>({
    subject1: '', subject2: '', interaction: 'berinteraksi harmonis', environment: 'taman bunga indah', style: 'CINEMATIC'
  });
  ...
  const handleGenerate = async () => {
    if (!subject1Image || !subject2Image) {
      setProcessing({ isProcessing: false, error: 'Harap unggah kedua foto subjek', progress: '' });
      return;
    }

    setProcessing({ isProcessing: true, error: null, progress: 'Merging Subjects...' });
    setResultImage(null);

    try {
      const result = await generateGabungScene({ ...config, subject1Image, subject2Image });
      setResultImage(result);
      setProcessing({ isProcessing: false, error: null, progress: '' });
    } catch (err: any) {
      setProcessing({ isProcessing: false, error: err.message || 'Gagal menggabungkan foto', progress: '' });
    }
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `gabung-orang-${Date.now()}.png`;
    link.click();
  };
    ...
    <div className="h-full bg-slate-50/50 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto min-h-full bg-white flex flex-col border-x border-slate-100">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Gabung Orang AI</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Neural Scene Synthesis</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ImageUploader 
                label="Subjek 1" 
                image={subject1Image} 
                onImageSelect={(img) => { setSubject1Image(img); setResultImage(null); }} 
                aspectRatio="square" 
                labelInside 
              />
              <ImageUploader 
                label="Subjek 2" 
                image={subject2Image} 
                onImageSelect={(img) => { setSubject2Image(img); setResultImage(null); }} 
                aspectRatio="square" 
                labelInside 
              />
            </div>
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <MapPin size={14} className="text-slate-300" /> 2. Latar Tempat
            </label>
            <input 
              type="text" 
              value={config.environment} 
              onChange={(e) => setConfig({...config, environment: e.target.value})} 
              placeholder="Misal: Taman bunga indah..." 
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 text-[11px] font-bold text-slate-700 focus:border-teal-500 outline-none transition-all shadow-inner placeholder:text-slate-300" 
            />
          </div>
          ...
          <div className="">
            <button
              ...
              disabled={processing.isProcessing || !subject1Image || !subject2Image}
              ...
              style={{ backgroundColor: processing.isProcessing || !subject1Image || !subject2Image ? undefined : primaryColor }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
              {processing.isProcessing ? (
                <span className="relative z-10">MERGING...</span>
              ) : (
                <span className="text-lg relative z-10">GABUNGKAN</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Gabung
              </label>
            </div>
              ...
              <AnimatePresence mode="wait">
                {processing.isProcessing ? (
                  ...
                  <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full relative select-none touch-none">
                    <img src={subject1Image!} className="absolute inset-0 w-full h-full object-cover grayscale opacity-20 blur-[1px]" alt="Original" />
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Gabung</h2>
