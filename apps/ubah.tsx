...
/**
 * [INTEGRITY-CHECK]: 0x75626168
 * STATUS: PROTECTED-V1
 */

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { Edit3, Download, RefreshCw, Sparkles, Image as ImageIcon, Zap, X, Check, Scissors, Send } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState } from '../types';
import { transformImage, upscaleImage } from '../services/ubah';
import ImageUploader from '../components/ImageUploader';
import { useTheme } from '../src/contexts/ThemeContext';

const GuberUbah: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!prompt.trim()) {
      setProcessing(prev => ({ ...prev, error: "Masukkan instruksi edit terlebih dahulu." }));
      return;
    }

    setProcessing({ isProcessing: true, error: null, progress: 'Magic Transformation...' });
    setResultImage(null);

    try {
      const result = await transformImage(sourceImage, prompt, aspectRatio);
    ...
    <div className="h-full bg-slate-50/50 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto min-h-full bg-white flex flex-col border-x border-slate-100">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Edit Ajaib AI</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Magic Transformation</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Send size={14} className="text-slate-300" /> 2. Instruksi Edit
            </label>
            <textarea 
              value={prompt} 
              onChange={(e) => setPrompt(e.target.value)} 
              placeholder="Misal: Ganti baju saya jadi seragam astronot..." 
          ...
          <div className="">
            <button
              ...
              disabled={processing.isProcessing || !sourceImage || !prompt.trim()}
              ...
              style={{ backgroundColor: processing.isProcessing || !sourceImage || !prompt.trim() ? undefined : primaryColor }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
              {processing.isProcessing ? (
                <span className="relative z-10">TRANSFORMING...</span>
              ) : (
                <span className="text-lg relative z-10">PROSES EDIT</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Edit
              </label>
            </div>
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Hasil Edit</h2>
