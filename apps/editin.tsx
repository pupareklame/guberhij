...
import { Sparkles, Download, RefreshCw, Image as ImageIcon, Zap, X, Check, Scissors, Send, MessageSquare } from 'lucide-react';
...
const GuberEditin: React.FC = () => {
  ...
  const [aiJoke, setAiJoke] = useState<string>('');
  ...
  const handleGenerate = async () => {
    ...
    setProcessing({ isProcessing: true, error: null, progress: 'Processing Edit...' });
    setResultImage(null);
    setAiJoke('');

    try {
      const result = await generateEditinResult(sourceImage, userRequest);
      setResultImage(result.imageUrl);
      setAiJoke(result.joke);
  ...
  const handleDownload = () => {
    ...
    link.download = `edit-lucu-${Date.now()}.png`;
    link.click();
  };
    ...
    <div className="h-full bg-slate-50/50 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto min-h-full bg-white flex flex-col border-x border-slate-100">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">Edit Lucu AI</h1>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-4">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <Send size={14} className="text-slate-300" /> 2. Permintaan Edit
            </label>
            <textarea 
              value={userRequest}
              onChange={(e) => setUserRequest(e.target.value)}
              placeholder="Misal: Edit saya jadi astronot yang lagi makan sate..."
          ...
          <div className="">
            <button
              ...
              disabled={processing.isProcessing || !sourceImage || !userRequest}
              ...
              style={{ backgroundColor: processing.isProcessing || !sourceImage || !userRequest ? undefined : primaryColor }}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/20 to-white/0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
              {processing.isProcessing ? (
                <span className="relative z-10">EDITING...</span>
              ) : (
                <span className="text-lg relative z-10">PROSES EDITIN</span>
              )}
            </button>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Editin
              </label>
            </div>
                    ...
                    {aiJoke && (
                      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
                        <div className="flex gap-2 items-start">
                          <div className="shrink-0 mt-0.5 text-teal-400">
                            <MessageSquare size={12} />
                          </div>
                          <p className="text-white text-[10px] font-medium italic leading-relaxed">"{aiJoke}"</p>
                        </div>
                      </div>
                    )}
                  </motion.div>
        ...
        {isCropping && resultImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col">
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">Crop Editin</h2>
