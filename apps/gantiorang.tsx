...
import { User, Users, Download, Scissors, Check, X, Sparkles, ImageIcon, Crop, Zap, Eye, Maximize, Recycle, Palette } from 'lucide-react';
import Cropper from 'react-easy-crop';
import { ProcessingState } from '../types';
import { swapPerson } from '../services/gantiorang';
import { upscaleImage } from '../services/geminiService';
import ImageUploader from '../components/ImageUploader';
import { useTheme } from '../src/contexts/ThemeContext';

const GuberGantiOrang: React.FC = () => {
  const { primaryColor } = useTheme();
  const [faceReferenceImage, setFaceReferenceImage] = useState<string | null>(null);
  ...
  // Crop States
  const [cropType, setCropType] = useState<'FACE' | 'POSE' | 'RESULT' | null>(null);
  ...
  const handleGenerate = async () => {
    if (!faceReferenceImage || !poseBaseImage) {
      setProcessing({ isProcessing: false, error: "Harap unggah kedua foto terlebih dahulu.", progress: '' });
      return;
    }
    ...
    setCropType(null);
    setProcessing({ isProcessing: true, error: null, progress: 'Melakukan Swap Wajah...' });
    
    try {
      const result = await swapPerson(poseBaseImage, faceReferenceImage, aspectRatio, similarity, style);
  ...
  const handleApplyCrop = async () => {
    ...
    if (cropType === 'FACE') sourceImage = faceReferenceImage;
    else if (cropType === 'POSE') sourceImage = poseBaseImage;
    else if (cropType === 'RESULT') sourceImage = resultImage;
    ...
    try {
      ...
      if (cropType === 'FACE') setFaceReferenceImage(base64Image);
      else if (cropType === 'POSE') setPoseBaseImage(base64Image);
  ...
  const handleDownload = () => {
    ...
    link.download = `swap-wajah-${Date.now()}.png`;
    link.click();
  };
    ...
    <div className="h-full bg-slate-50/50 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto min-h-full bg-white flex flex-col border-x border-slate-100 shadow-sm">
          ...
          <div className="flex items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-base font-black text-white tracking-tight leading-none mb-0.5 uppercase">FACE SWAP PRO</h1>
                <p className="text-[7px] font-bold uppercase tracking-[0.3em] leading-none text-white/60">Ganti Wajah, Tetapkan Pakaian</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          {/* Image Uploaders - Vertical Layout */}
          <div className="flex flex-col gap-6">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <User size={12} className="text-slate-300" /> 1. Foto Wajah (Referensi)
                </label>
                {faceReferenceImage && (
                  <button 
                    onClick={() => { setCropType('FACE'); setZoom(1); setCrop({x:0, y:0}); }}
              ...
              <ImageUploader
                ...
                onImageSelect={setFaceReferenceImage}
                onClear={() => setFaceReferenceImage(null)}
                aspectRatio="1-1"
                labelInside
              />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <ImageIcon size={12} className="text-slate-300" /> 2. Foto Pose & Badan (Target)
                </label>
                {poseBaseImage && (
                  <button 
                    onClick={() => { setCropType('POSE'); setZoom(1); setCrop({x:0, y:0}); }}
                    className="text-[9px] font-black text-blue-500 uppercase tracking-widest flex items-center gap-1 hover:text-blue-600 transition-colors"
                  >
                    <Scissors size={10} /> Crop Pose
                  </button>
                )}
              </div>
              <ImageUploader
                label="Unggah Foto Pose"
                image={poseBaseImage}
                onImageSelect={setPoseBaseImage}
                onClear={() => setPoseBaseImage(null)}
                aspectRatio="1-1"
                labelInside
              />
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Sparkles size={14} className="text-slate-300" /> Kemiripan Wajah ({similarity}%)
                </label>
              </div>
              <div className="px-2">
                <input
            ...
            <div className="space-y-3">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <Palette size={14} className="text-slate-300" /> Gaya Visual
              </label>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { id: 'PHOTOREALISTIC', name: 'Fotorealistik' },
                  { id: 'CINEMATIC', name: 'Cinematic' },
                  { id: 'STUDIO', name: 'Studio Portrait' },
                  { id: 'ARTISTIC', name: 'Artistik' },
                ].map((s) => (
                  <button
                    key={s.id}
          ...
          <div className="">
            <button
              ...
              disabled={processing.isProcessing || !faceReferenceImage || !poseBaseImage}
              ...
              style={{ 
                backgroundColor: processing.isProcessing || !faceReferenceImage || !poseBaseImage ? undefined : primaryColor,
              }}
            >
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={14} className="text-slate-300" /> Hasil Swap Wajah
              </label>
            </div>
            ...
            >
              <AnimatePresence mode="wait">
                {processing.isProcessing ? (
                  ...
                  >
                    <img src={poseBaseImage!} className="absolute inset-0 w-full h-full object-cover" alt="Original" />
                    ...
                    <div className="absolute bottom-4 left-4 px-2 py-0.5 bg-black/40 backdrop-blur-md rounded-full text-[6px] font-black text-white uppercase tracking-widest pointer-events-none">Foto Pose</div>
                    <div className="absolute bottom-4 right-4 px-2 py-0.5 bg-white/40 backdrop-blur-md rounded-full text-[6px] font-black text-black uppercase tracking-widest pointer-events-none">Hasil Swap</div>
                  </motion.div>
                ) : (
          ...
          >
            <div className="flex justify-between items-center p-6 border-b border-white/10">
              <h2 className="text-white font-black uppercase tracking-widest text-sm">
                {cropType === 'FACE' ? 'Crop Foto Wajah' : cropType === 'POSE' ? 'Crop Foto Pose' : 'Crop Hasil'}
              </h2>
            ...
            <div className="flex-1 relative">
              <Cropper
                image={cropType === 'FACE' ? faceReferenceImage! : cropType === 'POSE' ? poseBaseImage! : resultImage!}
                crop={crop}
                zoom={zoom}
                aspect={cropType === 'FACE' ? 1 : (aspectRatio === '1:1' ? 1 : aspectRatio === '3:4' ? 3/4 : aspectRatio === '4:3' ? 4/3 : aspectRatio === '9:16' ? 9/16 : 16/9)}
...
export default GuberGantiOrang;
