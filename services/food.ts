...
export const generateFoodThumbnail = async (image: string, title: string, style: string, typo: string, fx: string, aspectRatio: string = "9:16") => {
  try {
    const ai = getAI();
    const response = await ai.models.generateContent({
      ...
      contents: [{ 
        parts: [
          ...
          { text: `Create a professional food ad. Title: ${title}. Style: ${style}. Typography: ${typo}. Effects: ${fx}. 8K resolution.` }
        ] 
      }],
      config: { 
