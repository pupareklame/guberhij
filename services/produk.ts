...
import { GuberProdukConfig } from "../types";
...
export const generateProductScene = async (face: string, product: string, config: GuberProdukConfig, aspectRatio: string = "9:16") => {
  try {
    const ai = getAI();
    const response = await ai.models.generateContent({
      ...
      contents: [{ 
        parts: [
          ...
          { inlineData: { data: cleanBase64(product), mimeType: 'image/png' } },
          { text: `[PRODUCT AD TASK]: Create a high-end advertisement. Action: ${config.interactionState}. Angle: ${config.cameraAngle}. Outfit: ${config.outfitStyle}. Env: ${config.environment}. ${config.additionalPrompt}` }
        ] 
      }],
      config: { 
